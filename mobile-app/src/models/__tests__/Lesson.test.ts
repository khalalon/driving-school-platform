...
describe('isLessonInProgress / pickCurrentLesson (D-45)', () => {
  ...
  const at = (id: string, iso: string, status = LessonStatus.SCHEDULED, duration = 60): Lesson => ({
    ...base,
    id,
    status,
    scheduledDate: iso,
  ...
  it('en cours : entre le début et la fin (durée de la leçon), planifiée seulement', () => {
    expect(isLessonInProgress(at('a', '2026-09-20T10:30:00.000Z'), now)).toBe(true);
    expect(isLessonInProgress(at('b', '2026-09-20T09:30:00.000Z'), now)).toBe(false);
    expect(
      isLessonInProgress(at('c', '2026-09-20T09:30:00.000Z', LessonStatus.SCHEDULED, 90), now)
    ).toBe(true);
    expect(isLessonInProgress(at('d', '2026-09-20T11:00:00.000Z'), now)).toBe(false);
    expect(
      isLessonInProgress(at('e', '2026-09-20T10:30:00.000Z', LessonStatus.COMPLETED), now)
    ).toBe(false);
    expect(
      isLessonInProgress({ ...base, status: LessonStatus.SCHEDULED, scheduledDate: null }, now)
    ).toBe(false);
  });
  ...
  it('en cours d’abord, sinon la prochaine à venir, sinon rien', () => {
    ...
    expect(pickCurrentLesson([later, past, current, soon], now)?.id).toBe('current');
    expect(pickCurrentLesson([later, past, soon], now)?.id).toBe('soon');
    expect(
      pickCurrentLesson([past, at('done', '2026-09-20T12:00:00.000Z', LessonStatus.COMPLETED)], now)
    ).toBeNull();
