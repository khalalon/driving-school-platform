...
// `price` est un DECIMAL : pg le renvoie en chaîne, converti en nombre ici (contrat : `price` numérique).
const PRICING_COLUMNS = `id, school_id AS "schoolId", lesson_type AS "lessonType",
  price::float8 AS price, duration, created_at AS "createdAt", updated_at AS "updatedAt"`;
...
export class PricingRepository implements IPricingRepository {
  ...
  async setPricing(schoolId: string, data: SetPricingDTO): Promise<Pricing> {
    const result = await this.db.query<Pricing>(
      `INSERT INTO pricing (school_id, lesson_type, price, duration)
       ...
       DO UPDATE SET price = EXCLUDED.price, duration = EXCLUDED.duration, updated_at = NOW()
  ...
  async findBySchoolId(schoolId: string): Promise<Pricing[]> {
    const result = await this.db.query<Pricing>(
      `SELECT ${PRICING_COLUMNS} FROM pricing WHERE school_id = $1 ORDER BY created_at DESC`,
      [schoolId]
    );
    return result.rows;
  }
