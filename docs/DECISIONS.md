...
|---|---|---|
| **D-01** | **Le modèle de domaine retenu est celui du mobile pour le *flux* : l'élève *demande* (inscription, leçon, examen), l'instructeur *approuve et planifie*, puis *enregistre* (présence, résultat, paiement). Le backend s'aligne. Le modèle backend « admin crée des créneaux/sessions, l'élève s'y réserve » est abandonné comme flux principal — mais ses colonnes `capacity` / `current_bookings` sont conservées pour les séances de code collectives (D-21).** | Décision de l'auteur. Le mobile (13 k lignes, 20 écrans) est le produit. |
| D-02 | Les services sont fusionnés en **une seule application Express** (`services/api/`), les domaines restant des modules (`auth/`, `school/`, `student/`, `lesson/`, `exam/`, `payment/`, `notification/`) avec la même structure interne (routes → controllers → services → repositories). `analytics` n'est pas porté (D-31). | Phase 2. Une seule base, aucun appel inter-services autre que l'auth, 6 copies du même middleware. |
...
| **D-21** | Q-06 | **Une seule table `lessons`** : `status ∈ pending \| scheduled \| completed \| cancelled \| rejected`, **`requested_date`** (souhaitée par l'élève, saisie dans `BookLessonScreen`) et **`scheduled_date`** (confirmée par l'instructeur), deux colonnes distinctes. `student_id` (élève demandeur, D-28) sur la leçon. **Les séances `CODE` sont collectives** : `capacity > 1`, `current_bookings` conservés ; une leçon `Manœuvre` / `Parc` a `capacity = 1`. Complété par D-32 (instructeur). | Deux colonnes : ne pas confondre « aucune date demandée » et « date non confirmée ». Une seule table évite une migration de données à l'approbation. | Le détail du rattachement d'un élève à une séance `CODE` collective existante (plutôt que création d'une leçon par demande) dépend de **Q-06b**. |
...
| **D-31** | Q-16 | **`analytics` : supprimé** (middleware pass-through = faille, aucun écran). **`payment` : porté, non monté.** **`notification` : voir Q-16b.** | Éliminer la faille plutôt que la surveiller ; le paiement est manuel. | `analytics` retiré de compose, Makefile, CI, Nginx en Phase 2 ; le code reste dans l'historique git. |
...
### Q-06b — Séances de code (`CODE`) : collectives ou individuelles selon l'école ?
D-21 acte que les séances `CODE` sont collectives (`capacity > 1`). Reste à confirmer que c'est vrai dans les 5 écoles pilotes, et à fixer le mécanisme de rattachement, car le mobile envoie aujourd'hui une demande **vers un instructeur** sans notion de séance existante.
- **(a)** Toujours collectives : une demande `CODE` `pending` est rattachée par l'instructeur à une séance `CODE` `scheduled` existante de son école (ou en crée une) ; la présence est marquée **par élève** (`PUT /api/lessons/:id/attendance { studentId, attended, … }`). Les leçons `Manœuvre` / `Parc` restent individuelles (`capacity = 1`, présence sans `studentId`).
- **(b)** Collectives ou individuelles selon l'école : l'instructeur choisit à l'approbation (`capacity` saisie) ; même mécanisme de présence par élève que (a).
- **(c)** Individuelles en v1 (`capacity = 1` pour tous les types) ; le collectif viendra plus tard.

Bloque : 3.4, 5.2, 5.3, 5.4, 6.3.

### Q-16b — Notifications push en v1 ?
Constat (16/09) : le mobile n'a **aucun** code de notification — pas de dépendance `expo-notifications`, aucun enregistrement de token, aucun appel à `/api/notifications`. Ce n'est pas un stub, c'est absent. Les notifications « ta demande a été approuvée » seraient une **fonctionnalité nouvelle** de bout en bout (permission, token Expo, table `push_tokens`, envoi à l'approbation, écran).
- **(a)** Dans la v1 : le module `notification` est monté, la table `push_tokens` créée (3.3), le mobile enregistre le token au login, l'approbation d'inscription / leçon / examen envoie un push. Ajoute ~3 tâches au plan (Phase 5 et 6).
- **(b)** In-app seulement en v1 : le module est porté mais **non monté** (comme `payment`), pas de table, l'élève voit le nouveau statut en ouvrant l'app. La Phase 3 s'allège (3.3 annulée).
- **(c)** Supprimé comme `analytics`.

Bloque : 2.5 (montage ou non), 3.3.
