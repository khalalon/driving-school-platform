...
| A2 | POST | `/api/auth/register` | `RegisterScreen`, `InstructorRegistrationScreen` (une seule étape après 6.2) | `{ email, password, firstName, lastName, schoolCode?, phone?, licenseNumber? }` (public). Sans `schoolCode` → rôle `student`. Avec `schoolCode` valide → rôle = `school_codes.role`, `phone` et `licenseNumber` **requis**, ligne `instructors` créée, code consommé (D-17). Le champ `role` n'est **plus accepté**. | 201 `{ accessToken, refreshToken }` ; 400 `INVALID_SCHOOL_CODE` si code inconnu/expiré/épuisé ; 409 `CONFLICT` si email pris | **DIVERGE** | Backend actuel : accepte `{ email, password, role, firstName, lastName }` — les noms sont **exigés et enregistrés** (3.1, D-16), mais `role` reste requis et libre (y compris `admin`) et `schoolCode` est inconnu. Mobile actuel : envoie `role`, jette `firstName`/`lastName`. Tâches 4.1, 4.2, 6.2. |
