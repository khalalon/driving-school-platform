...
**Statut : VALIDÉ le 17/09/2026** (réponses aux questions ouvertes et aux 15 questions de comportement appliquées, voir `DECISIONS.md` D-16 à D-33). Restent suspendus : Q-06b (séances `CODE` collectives), Q-16b (notifications), Q-17 (leçon payée annulée), Q-18 (absence facturée), Q-19 (procédure d'examen ATTT) ; marqués dans les lignes concernées.
...
| L5 | PUT | `/api/lessons/:id/approve` | `LessonRequestsScreen` — **stub aujourd'hui** | `{ scheduledDate (ISO, futur), durationMinutes, price?, adminNotes? }` (+ `capacity` si Q-06b = b) — **tout instructeur de l'école** ; celui qui approuve devient `instructorId` | `Lesson` (`scheduled`, `instructorId` = appelant, `price` = `pricing[school, type]` si un tarif existe, sinon le `price` saisi ; **400 `PRICE_REQUIRED`** si ni l'un ni l'autre). Le montant est stocké sur la leçon dans les deux cas (D-30) | **MANQUE** | `lessonService.approveLesson` du mobile envoie `{ startTime, endTime, adminNotes }` → réécrit en 6.3. Tâche 5.3. Dépend de Q-06b pour `CODE`. |
...
| L7 | PUT | `/api/lessons/:id/attendance` | `TodayLessonsScreen` — **non câblé** | `{ attended, feedback?, rating? }` par **identifiant de leçon** ; pour une séance collective `CODE` : `+ studentId` (Q-06b) — **uniquement l'instructeur de la leçon** (`instructorId` = appelant, sinon 403) | `Lesson` (`completed`) ; `student_lesson_stats.completed_lessons` **incrémenté seulement si `attended = true`**. Facturation d'une absence : **Q-18** | **DIVERGE** | Backend actuel : `PUT /bookings/:bookingId/attendance` par identifiant de réservation. Tâche 5.4, 6.3. Dépend de Q-06b. |
...
| X3 | PUT | `/api/exams/:id/schedule` | `ExamRequestsScreen` — **stub** | `{ dateTime (ISO, futur), location }` — instructeur de l'école | `Exam` (`scheduled`) | **MANQUE** | Pas de colonne `location` en base (3.5). Tâches 5.6, 6.5. **Suspendu à Q-19** pour le libellé (« planifier » vs « enregistrer la date de session ») ; le payload ne change pas. |
...
| P2 | GET | `/api/profiles/:studentId/schools/:schoolId/lessons` | `StudentLessonsTab` | — | `LessonHistory[]` `{ id (lesson), type, scheduledDate, durationMinutes, instructorFirstName, instructorLastName, attended?, feedback?, rating?, paid, amount?, paymentDate?, paymentMethod? }` | **DIVERGE** | Idem P1 ; après 3.4 la source est `lessons` (plus `lesson_bookings` pour l'individuel). |
| P3 | GET | `/api/profiles/:studentId/schools/:schoolId/exams` | `StudentExamsTab` | — | `ExamHistory[]` `{ id (exam), type, dateTime, result, score?, notes?, paid, amount?, paymentDate?, paymentMethod? }` | **DIVERGE** | Idem P1 ; après 3.5 la source est `exams`. |
...
| P6 | PUT | `/api/profiles/lessons/:lessonId/mark-paid` | `StudentLessonsTab` | `{ amount, paymentMethod ∈ cash\|card\|bank_transfer }` | 204 | **DIVERGE** | Chemin actuel `/bookings/:bookingId/mark-paid` sur `lesson_bookings` ; après 3.4 le paiement est porté par `lessons` (individuel) → chemin renommé. Tâche 5.0. |
| P7 | PUT | `/api/profiles/exams/:examId/mark-paid` | `StudentExamsTab` | `{ amount, paymentMethod }` | 204 | **DIVERGE** | Chemin actuel `/registrations/:registrationId/mark-paid` ; après 3.5 porté par `exams` → renommé. Tâche 5.0. |
...
**Suspendu à Q-16b.** Le mobile n'a aucun code de notification (vérifié le 16/09 : pas de dépendance `expo-notifications`, aucun appel). Si Q-16b = (a), les lignes suivantes entrent au contrat : `POST /api/notifications/push-tokens { token, platform }`, `DELETE /api/notifications/push-tokens/:token`, `GET /api/notifications`, `PUT /api/notifications/:id/read`, et un envoi automatique à E5, E6, L5, L6, X3, X4. Sinon : aucune ligne.

Routes backend existantes dans `services/notification` (`/send`, `/send-bulk`, `/`, `/unread-count`, `/:id`, `/:id/read`, `/read-all`, `/preferences/me`, `/preferences`, `/push-tokens`, `/push-tokens/:token`) : les routes `preferences` et `push-tokens` échouent en SQL (tables absentes).
...
Suspendu : L5, L7 (Q-06b) ; L3 (Q-17, sort d'une leçon payée annulée) ; L7 (Q-18, facturation d'une absence) ; X3, X4 libellés (Q-19, procédure ATTT) ; §7 (Q-16b).
