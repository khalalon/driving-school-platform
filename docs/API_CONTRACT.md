...
Objet `Lesson` (cible) : `{ id, schoolId, preferredInstructorId?, instructorId?, instructor?: { id, firstName, lastName }, studentId (users.id), student: { id, firstName, lastName }, type, status, requestedDate?, scheduledDate?, durationMinutes?, price?, capacity, currentBookings, notes?, adminNotes?, rejectionReason?, attended?, feedback?, rating?, paid, amount?, paymentDate?, createdAt, updatedAt }`. `instructorId` / `instructor` sont `null` tant que la leçon est `pending`.
...
|---|---|---|
| EXISTE | 29 | A1–A5, S1–S4, S6, E1–E6, L1, L2, P1–P11 |
| DIVERGE | 2 | L3, L7 |
| MANQUE | 8 | L4, L5, L6, X1–X5 |
| SUPPRIMÉE | 2 | S5, L8 |
