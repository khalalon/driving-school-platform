...
|---|---|---|---|---|---|---|---|
| A1 | POST | `/api/auth/login` | `LoginScreen` → `AuthContext.login` | `{ email, password }` (public) | `{ accessToken, refreshToken }` ; le mobile enchaîne sur A3 pour le profil | **EXISTE** | Jetons D-12 / D-23 depuis 4.4 : `accessToken` 1 h et `refreshToken` 30 j, claim `type`, deux secrets. Aujourd'hui le mobile décode le JWT au lieu d'appeler A3 et ignore `refreshToken` (4.5, 6.2). |
...
| A4 | POST | `/api/auth/refresh` | `ApiClient` interceptor sur 401 (4.5) | `{ refreshToken }` (public) | `{ accessToken, refreshToken }` — **nouveau** refresh, l'ancien est révoqué (rotation, D-12) | **DIVERGE** | Backend actuel : renvoie une paire et refuse un access token présenté comme refresh (claim `type`, secret distinct — 4.4), mais ne révoque rien (pas de rotation). Tâche 4.6. |
