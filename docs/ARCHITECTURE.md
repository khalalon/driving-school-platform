...
| `exam` | `/api/exams` | modèle D-01 / D-33 : X2 (demande `pending`, école résolue depuis la fiche `students`, 403 `NOT_ENROLLED`, pas d'éligibilité — D-26), X1 (élève : les siens ; instructeur : tous ceux de son école ; admin : tout), `GET /:id` scoped (5.5) ; X3–X5 en 5.6 ; anciennes sessions et `exam_registrations` abandonnées | `student` (`StudentRepository`), `school` (`InstructorRepository`), `SchoolGuard` |
