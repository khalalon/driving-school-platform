...
| `lesson` | `/api/lessons` | modèle D-21 / D-32 : L2 (demande `pending`, école résolue depuis la fiche `students`, 403 `NOT_ENROLLED`), L1 (portée par appelant : élève, file `pending` de l'école et/ou leçons de l'instructeur, admin), `GET /:id` scoped (5.2) ; L3, L5, L6 en 5.3, L4, L7 en 5.4 ; anciennes routes de créneaux et `lesson_bookings` abandonnées | `student` (`StudentRepository`), `school` (`InstructorRepository`, grille D-30 en 5.3), `SchoolGuard` |
