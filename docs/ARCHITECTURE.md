...
| `lessons` (007) | `school_id`, **`student_id` → students NOT NULL** (une leçon = un élève, D-34), `instructor_id` nullable (renseigné à l'approbation, D-32), `preferred_instructor_id`, `type` ∈ CODE/Manœuvre/Parc, `status` ∈ **pending**/scheduled/completed/cancelled/**rejected** (défaut `pending`), `requested_date`, `scheduled_date` (ex-`date_time`, nullable), `duration_minutes` nullable, `price`, `capacity` **CHECK (= 1)**, `current_bookings`, `notes`, `admin_notes`, `rejection_reason`, `attended`, `feedback`, `rating`, `paid`, `amount`, `payment_date`, `payment_method` | lesson | lesson, student |
| `lesson_bookings` | `lesson_id`, `student_id` → **students**, `attended`, `feedback`, `rating`, + (003) `paid`, `payment_date`, `payment_method`, `amount` ; unique(lesson, student). **Données reprises dans `lessons` par 007, table conservée mais plus alimentée** (anciennes routes `/book` jusqu'en 5.2, P2–P7 jusqu'en 5.0, éligibilité exam jusqu'en 3.4) | lesson (ancien `/book`), student | lesson, exam, student |
