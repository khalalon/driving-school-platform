...
Thème (D-48, 11.1) : `src/theme` = jetons sémantiques (`surface`, `surfaceRaised`, `textPrimary`, `accent`, `danger`…) déclinés en **clair** (`light.ts`) et **sombre** (`dark.ts`) à partir d'une palette unique (`tokens.ts`), plus les échelles d'espacement, de rayons et de typographie ; `ThemeProvider` suit `useColorScheme()` et les écrans lisent `useTheme()`. Deux tests gardent l'ensemble : parité des jetons entre thèmes et **contraste ≥ 4,5:1** des paires texte/fond (c'est lui qui a imposé un texte foncé sur les couleurs vives du thème sombre).

Composants partagés (D-48, 11.2) : `mobile-app/src/components/ui/` fournit `Screen`, `AppBar`, `Button`, `Card`, `Chip`, `Badge`, `Field`, `SectionHeader`, `ListRow`, `EmptyState`, `Skeleton` / `SkeletonCard` et `Toast`, importés par le seul `components/ui/index.ts`. Aucun composant ne code une couleur : un statut choisit une **intention** (`neutral`, `accent`, `success`, `warning`, `danger`, `tones.ts`) et le thème décide du rendu. Les tests rendent chaque composant dans les deux thèmes et relisent les couleurs posées dans l'arbre : une couleur étrangère au thème fait échouer la suite. `ThemeProvider` accepte un `name` pour forcer un thème (accroche des tests) ; hors `ThemeProvider`, `useTheme()` renvoie le thème clair.

Retours d'action (D-48, 11.5) : `ToastProvider` (`src/context/ToastContext.tsx`) est monté dans `App.tsx` entre la langue et la session ; un écran annonce un succès par `useToast().showToast(message)` — un seul toast à la fois, effacé après 3 s. `Alert.alert` est réservé aux décisions (annuler une leçon, refuser une demande, approuver) et aux erreurs bloquantes. Hors fournisseur, `useToast()` renvoie des fonctions sans effet : un test d'écran isolé n'a rien à monter.

Thème sombre en production (D-48, 11.6) : `app.json` déclare `userInterfaceStyle: "automatic"` — sans cela le réglage du téléphone n'atteint jamais `useColorScheme()` et l'application reste claire. L'icône, l'icône adaptative et les deux écrans de démarrage (clair et sombre) sont dessinés aux couleurs des jetons ; le script de génération vit dans l'historique de la tâche, les PNG sont versionnés dans `mobile-app/assets/`. Accessibilité : les composants cliquables portent un rôle et un libellé, et mesurent au moins 44 px de haut, débord (`hitSlop`) compris — vérifié par `components/ui/__tests__/accessibility.test.tsx`.

Langues (D-47, 10.1) : catalogue `mobile-app/src/i18n/{fr,ar}.ts` (mêmes clés, test de parité), langue courante au niveau du module (`t()` utilisable hors composant, par `models/` et `utils/format.ts`), `LanguageProvider` au-dessus de la navigation, sélecteur `components/LanguagePicker` sur l'écran de connexion et dans « My Profile ». Premier lancement : langue du téléphone (`ar*` → arabe, sinon français), puis choix mémorisé dans `AsyncStorage` (`@driving_school/language`). L'arabe passe l'app en **RTL** (`I18nManager.forceRTL` + `Updates.reloadAsync()`) ; les marges directionnelles utilisent `marginStart` / `marginEnd` (retournées par React Native) et les icônes porteuses de sens passent par `mirrorIcon` (`src/utils/rtl.ts`), qui se fonde sur la **langue choisie** et non sur `I18nManager.isRTL` (constante native en retard d'un redémarrage). Les erreurs du serveur sont traduites par code stable (`ApiError`, 10.5).
